...
# Original: outputs/florida_ed_concordance_analysis_20260726/scripts/RUN_OUTCOME_SPECIFIC_PRIMARY.ps1
...
$Scratch = Join-Path $Temp 'outcome_specific_model_scratch'
...
foreach ($sample in $samples) {
    foreach ($cohort in @('race', 'sex_gender')) {
        ...
        Invoke-Step 'build' "07_prepare_outcome_specific_${cohort}_$($sample.Id)" @(
        ...
        Invoke-Step 'estimate' "08_estimate_outcome_specific_${cohort}_$($sample.Id)" @(
            (Join-Path $Scripts '08_estimate_primary_models.py'),
            '--matrix-root', $MatrixRoot,
            '--matrix-id', $matrixId,
            '--scratch', (Join-Path $Scratch $sample.Id),
            '--output', (Join-Path $Phase2 "results\outcome_specific_primary\$($sample.Id)"),
            '--cohort', $cohort,
            '--bootstrap-draws', '9999',
            '--seed', '20260726'
        )
        $auditId = "outcome_specific_${cohort}_$($sample.Id)"
        Invoke-Step 'audit' "30e_audit_${auditId}" @(
            (Join-Path $Scripts '30e_checkpoint_primary_matrix_audit.py'),
            '--phase2', $Phase2,
            '--matrix-root', $MatrixRoot,
            '--matrix-id', $matrixId,
            '--primary-scratch', (Join-Path $Scratch $sample.Id),
            '--scratch-id', $cohort,
            '--results-root', (Join-Path $Phase2 "results\outcome_specific_primary\$($sample.Id)"),
            '--cohort', $cohort,
            '--audit-id', $auditId,
            '--expected-analysis-sample', $sample.Policy,
            '--expected-eligibility-policy', 'primary',
            '--expected-outcome', $sample.Outcome,
            '--expected-confirmatory', 'true',
            '--row-chunk', '333333'
        )
        Invoke-Step 'audit' "32_compact_${auditId}" @(
            ...
            '--matrix-root', $MatrixRoot,
            '--matrix-id', $matrixId,
            '--scratch-dir', (Join-Path (Join-Path $Scratch $sample.Id) $cohort),
            '--execute'
        )
    }
}

Invoke-Step 'audit' '30f_aggregate_outcome_specific_results_audit' @(
    (Join-Path $Scripts '30f_aggregate_model_audit_checkpoints.py'),
    '--phase2', $Phase2,
    '--expected-audit-ids', (
        'outcome_specific_race_los,' +
        'outcome_specific_sex_gender_los,' +
        'outcome_specific_race_charge,' +
        'outcome_specific_sex_gender_charge'
    ),
    '--output-stem', 'independent_outcome_specific_results_audit',
    '--audit-id', 'outcome_specific_primary_results_audit_v2'
)

"[$(Get-Date -Format o)] RUN_OUTCOME_SPECIFIC_PRIMARY completed successfully." |
    Tee-Object -FilePath (Join-Path $Logs 'RUN_OUTCOME_SPECIFIC_PRIMARY_COMPLETE.log')
